...
#define TASK_PRIORITY_SSDVT             (KAL_PRIORITY_CLASS0 + 1)    /* Will only be used in DVT load */
...
#define TASK_PRIORITY_MED               (KAL_PRIORITY_CLASS8 + 5)
...
/*************************Task CFG Begin****************/
#if defined(__CMUX_SUPPORT__) || defined(__USB_TETHERING__) || defined(__MBIM_SUPPORT__)
...
#if defined(__USB_TETHERING__) || defined(__MBIM_SUPPORT__)
...
task_stack_size(1118)
task_create_function(cmux_create)
task_stack_internalRAM(KAL_FALSE)
task_external_queue_size(64)
...
#elif (defined(__SINGLE_BANK_NOR_FLASH_SUPPORT__) || defined(__NOR_IDLE_SWEEP__)) && defined(__MTK_TARGET__)
...
#if defined(__SSDVT_TEST__)
/*task_indx_type*/
task_index(INDX_SSDVT) 
/*dummy mod is specified to get task id by mod id, use mod_task_g*/
task_module_map(INDX_SSDVT,MOD_DUMMY_SSDVT)
/*module_type and mod_task_g*/
task_module_map(INDX_SSDVT, MOD_SSDVT)

/*task's parameters*/
task_name("SSDVT")
task_queue_name("SSDVT Q")
task_priority(TASK_PRIORITY_SSDVT, KAL_TASK_INITIAL_PRIORITY)
task_stack_size(2048)
task_create_function(ssdvt_create)
task_stack_internalRAM(KAL_FALSE)
task_external_queue_size(0)
task_internal_queue_size(0)
task_boot_mode(NORMAL_M)
