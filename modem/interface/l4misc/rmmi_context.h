...
{
    kal_uint32 cell_id;     // for +CREG and +CGREG, high 16 bits for RNC id, low 16 bits for cell_id
    kal_uint16 lac_value;   // for +CREG and +CGREG
#ifdef __REPORT_RAC__
    kal_uint8 rac_value;    // for +CGREG
#endif
    kal_uint8 act;          //[MAUI_01360129]
    kal_uint8 state;    //for +CREG and +CGREG
    kal_uint8 cause;
...
typedef struct rmmi_clip_info_struct
{
    ...
    kal_uint8 type;
    kal_uint8 subaddr[MAX_CC_SUB_ADDR_LEN];      //MAX_CC_ADDR_LEN
    kal_uint8 satype;
...
typedef struct
{
    kal_uint8 plmn[7]; //ASCII string type PLMN ID
    kal_uint16 lac;
    kal_uint32 cell_id;
} rmmi_ecellid_struct;
#endif
...
#endif
    kal_uint8 register_act;                   // as combine cs/ps
...
/* under construction !*/
   ...
   kal_uint32 atipr_rate;
    ...
    kal_uint8 egind_mode;
#endif

#ifdef __PPE_INTERFACE_SUPPORT__
    kal_uint8 ppe_action[PPE_MAX_TYPE];
#endif

#if defined(__ECELLID_SUPPORT__) || defined(__PPE_INTERFACE_SUPPORT__)
    rmmi_ecellid_struct ecellid_info;
#endif

	kal_bool enable_ecell_ext3_ext4;

    kal_uint8 screen_state;
} rmmi_context_struct;
