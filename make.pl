...
@actions = qw(getusr new update remake clean codegen nvram_auto_gen bootloader emiclean emigen sysgen sys_auto_gen sys_mem_gen ckmemlayout drv_feature_check viewlog rel c,r c,u ckcr removecode scan check_scan check check_dep remake_dep update_dep clean_codegen slim_codegen slim_update genlog findpad elfpatch gendsp cksysdrv nvram_auto_gen cust_menu_tree_check genmoduleinfo gen_modlibtbl ximgen gen_bt_switch_info gendummylis gendummysym echo_dspinfo video_mem_gen umts_gen asngen asngen_l4 asnregen slim_mcddll memquery);
...
$projectFolder = "build/".uc "${project_name}"."/$flavor";
if ($chk_cmd == 1) 
{
print "\n$projectFolder\n";
exit 0;
...
if ((-e "label.ini") && (!-e "make/rel/hq/USR.txt")) {
	...
	print "perl mtk_tools/USR/CBrUSR_Initial.pl $project_name $flavor $original_flavor\n";
...
$gen_concate_option = $gen_concate_option . " NO_PCIBT=TRUE";
...
my $return_code = system("${makeCmd} -f${makeFolder}${myMF} -r -R PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor $gen_concate_option gen_concatenate_proj");
...
my $strBypassMoDIS = 0;
my $notify_list;
$notify_list = "\\\\glbfs14\\sw_releases\\3rd_party\\Scripts\\BM_conf.ini" if($^O eq "MSWin32");
...
if ($demo_project eq "TRUE") {
  print ("perl make/flavor_conf.pl \"$theMF\" $original_flavor $tmpFolder/cus_folder.tmp\n");
  $fla_rel = system("perl make/flavor_conf.pl \"$theMF\" $original_flavor $tmpFolder/cus_folder.tmp> $logFolder/flavor_conf.log 2>&1");
  die "\nError: make/flavor_conf.pl failed, please check error message in $logFolder/flavor_conf.log\n" if($fla_rel);
}
...
# if it is too long, it will cause build error. The max length is defined 120 characters.
chomp($cwd = cwd());
$len = length($cwd);
$folder_limit = 120;
...
foreach my $action (@theAct) {
  ...
  if ($action eq "clean") {
    ...
    if ($action !~ /^ckmake$/i) {
      $result = system("${makeCmd} -f${makeFolder}${myMF} -r -R PROJECT_NAME=$project_name \"PROJECT_MAKEFILE=$theMF\" FLAVOR=$flavor ckmake");
...
{
	my $modis_action = shift @_;
	my $result_projectFolder = "build/".uc "${project_name}"."/$flavor";
