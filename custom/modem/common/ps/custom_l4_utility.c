    ...
    *cfun_state = 1;
    ...
    if ((query_ps_conf_test_mode()==PS_CONF_TEST_FACTORY) && (query_ps_conf_test_profile_setting() & FACTORY_MODEM_AUTO_BOOT_AND_AUTO_ANSWER_BIT_FOR_L4C))
    {
        ...
        *cfun_state = 4;
        ...
        #if defined(__MULTIPLE_PPP_DIALUP_SUPPORT__)//TRIPLE_PPP for MT6280 IOT
            *cfun_state = 4; //20120917, Support SPEECH(by Data Card Tool) + TRIPLE PPP IOT test cases
        #endif
...
#ifdef __CUST_TDD_SIG_LEVEL__
#define CUSTOM_3G_TDD_RSCP_THRESHOLD_1 (-85)
#define CUSTOM_3G_TDD_RSCP_THRESHOLD_2 (-90)
#define CUSTOM_3G_TDD_RSCP_THRESHOLD_3 (-95)
#define CUSTOM_3G_TDD_RSCP_THRESHOLD_4 (-100)
                ...
                umts_rrc_state_enum current_state)
{
    ...
    // RRC state changes from IDLE/INACTIVE to CONNECTED
    if ( (previous_state == UMTS_RRC_IDLE || previous_state == UMTS_RRC_INACTIVE)
        && (current_state >= UMTS_RRC_CELL_FACH && current_state <= UMTS_RRC_CELL_DCH)
       )
    {
        string_length = rmmi_fast_string_print(buffer,(kal_uint8*) "+ERRC: ON", arg_list, 1);
        l4c_ppe_proxy_urc_pind(buffer, string_length);
    }
    // RRC state changes from CONNECTED to IDLE/INACTIVE
    else if ( (previous_state >= UMTS_RRC_CELL_FACH && previous_state <= UMTS_RRC_CELL_DCH)
        && (current_state == UMTS_RRC_IDLE || current_state == UMTS_RRC_INACTIVE)
       )
    {
        string_length = rmmi_fast_string_print(buffer,(kal_uint8*) "+ERRC: OFF", arg_list, 1);
        l4c_ppe_proxy_urc_pind(buffer, string_length);
...
{
	/* ECC_CATEGORY_SUPPORT, default value : KAL_FALSE.
...
/*****************************************************************************************
* FUNCTION
*   custom_rac_display_searching_when_plmn_loss()
*
* DESCRIPTION
*   When 3G PS OOS occur, UE may display searching for a long time.
*   This function decide whether to display searching when PLMN LOSS
*
* PARAMETERS
*   NONE
*
* RETURNS
*   KAL_TRUE    When PLMN Loss/OOS, UE MAY display searching
*   KAL_FALSE   When PLMN Loss/OOS, UE never display searching
*   
******************************************************************************************/
kal_bool custom_rac_display_searching_when_plmn_loss(void)
{
#ifndef __NOT_DISPLAY_SEARCHING_WHEN_OOS__
    return KAL_TRUE;
#else
    return KAL_FALSE;
#endif
...
/*****************************************************************************
 * FUNCTION
 *  custom_check_if_ipv4v6_fallback_needed
 * DESCRIPTION
 *  Check from the NW cause if needs to do fallback and retry IPv4/IPv6 again.
 * RETURNS
 * KAL_TURE : try IPv4 and IPv6 again  
 * KAL_FALSE : no more try.
 * USAGE
 *****************************************************************************/     
kal_bool custom_check_if_ipv4v6_fallback_needed(kal_uint16 tcm_act_rej_cause)
{

  /*
    ** The customer have to remove the below "return KAL_TRUE" in order to follow the operator's requirement.
    ** and make a choice of what they want as Reject Casue.
    ** return KAL_TRUE means that IPv4 only or IPv6 only  Retry Mechanism will go in all NW causes 
    **     V VVVVVVVV  */ 
    // return KAL_TRUE;
  // 

  switch(tcm_act_rej_cause)
  {

/*  the customer can add or remove reject causes which they want.
**	  case XXX "  refer to Ps_public_enum.h
**/
//    case INSUFF_RESOURCE :
//    case UNKNOWN_APN :
      case UNKNOWN_PDP_ADDR_OR_TYPE :
//    case AUTHENTICATION_FAILURE :
//    case ACTIVATION_REJ_GGSN :
//    case ACTIVATION_REJ :
//    case UNSUPPORTED_SERVICE_OPTION	:
//    case UNSUBSCRIBED_SERVICE_OPTION :
//    case OUT_OF_ORDER_SERVICE_OPTION :
#ifdef __CONSIDER_CAUSE52_FOR_IPV4V6_FALLBACK__
      case SINGLE_ADDR_BEARERS_ONLY_ALLOWED :
#endif
    {
        /* it needs to try to IPv4 or IPv6 again.
             */
         return KAL_TRUE;
         break;
    }
...
/*****************************************************************************
* FUNCTION
*   custom_on_screen_off_send_min_info_creg()
*
* DESCRIPTION
*   This function is used to turn on/off sending min info for creg on screen off
*
* PARAMETERS
*   none
*
* RETURNS
*   TRUE    : Customize the CREG URC indication to send minimum information when screen is off
*   FALSE   : No customization
*****************************************************************************/
kal_bool custom_on_screen_off_send_min_info_creg(void)
{
	/* on_screen_off_send_min_info_creg, default value : KAL_FALSE.
*   KAL_TRUE    : Customize the CREG URC indication to send minimum information when screen is off
*   KAL_FALSE   : No customization
*/
}
