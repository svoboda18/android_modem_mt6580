...
ifeq ($(strip $(COMPILER)),GCC)
    ...
    LNKOPT += --start-group -lnosys -lm -lc -lgcc $(strip $(LINK_OBJ_LIST)) $(strip $(LINK_LIB_LIST)) --end-group
    LNKOPT := $(subst \,/,$(LNKOPT))
endif
...
ifeq ($(strip $(DO_UPDATE_MODULE)),TRUE)
  ...
  COMPLIBLIST = $(foreach comp,$(UPDATE_MODS),$(comp).a)
else
  ifeq ($(strip $(DO_REMAKE_MODULE)),TRUE)
    ...
    COMPLIBLIST = $(foreach comp,$(REMAKE_MODS),$(comp).a)
  else
...
ifeq ($(strip $(COMPILER)),GCC)
  BTLDLNKOPT    += --start-group -lnosys -lm -lc -lgcc $(strip $(BL_LINK_OBJ_LIST)) $(strip $(BL_LINK_LIB_LIST)) -lbootloader --end-group
  BTLDLNKOPT    := $(subst \,/,$(BTLDLNKOPT))
  
  BTLDEXTLNKOPT += --start-group -lnosys -lm -lc -lgcc $(strip $(BL_LINK_OBJ_LIST)) $(strip $(BL_LINK_LIB_LIST)) -lbootloader_ext --end-group
	...
	@$$(IF_EXIST) $$(FIXPATH)/tools/ckMemLayout.pl $$(THEN)\
		$$(IF_EXIST) $$(strip $(TARGDIR))/$(2) $$(THEN)\
			$$(IF_EXIST) $$(strip $(TARGDIR))/$(3) $$(THEN)\
				(perl $$(FIXPATH)/tools/ckMemLayout.pl 2 TRUE $(4) "$$(strip $(TARGDIR))/$(2)" $$(TMPDIR)/~flash_cfg_tmp.c "$$(strip $$(PROJECT_MAKEFILE))" $$(strip $$(BUILD_SYSDIR)) $$(strip $$(INSIDE_MTK)) > $$(strip $$(COMPLOGDIR))/ckmemlayout2_$(1).log  2>&1)$$(WITH) \
...
ifneq ($(filter __VI_ENABLED__,$(strip $(COM_DEFS))),)
	...
	@if exist $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log $(DEL_FILE) $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(IF_EXIST) $(strip $(COMPLOGDIR))/CheckBinaryBlockUsage.log $(THEN) ($(DEL_FILE) $(strip $(COMPLOGDIR))/CheckBinaryBlockUsage.log) $(CMD_END)
...
ifeq ($(strip $(COMPILER)),GCC)	
	@perl tools/C_Library/cLib_stacksize_shrink.pl $(strip $(DUMP)) $(strip $(TARGDIR))/$(IMG_FILE) $(strip $(TARGDIR))/$(LIS_FILE) $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))/cLib_stacksize_shrink.log 2>&1 $(WITH) \
	($(IF_ERR_TRUE) $(THEN) (echo Error: Failed in cLib_stacksize_shrink.pl. Please check $(strip $(COMPLOGDIR))/cLib_stacksize_shrink.log $(WITH) exit 1) $(CMD_END))
endif
...
ifneq ($(strip $(NAND_FLASH_BOOTING)),NONE)
	@$(IF_EXIST) ./tools/CheckBinaryBlockUsage.pl $(THEN)\
		(perl ./tools/CheckBinaryBlockUsage.pl $(strip $(TMPDIR))/~flash_cfg_tmp.c $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(TARGDIR))/$(strip $(BTLD_BIN_FILE)) $(strip $(TARGDIR))/$(strip $(BTLD_EXT_BIN_FILE)) $(strip $(PLATFORM)) $(strip $(BUILD_SYSDIR)) "$(strip $(PROJECT_MAKEFILE))" > $(strip $(COMPLOGDIR))/CheckBinaryBlockUsage.log 2>&1) $(WITH) \
...
ifeq ($(strip $(PLATFORM)),MT6570)
	@$(IF_EXIST) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(THEN)\
		...
		$(MOVE) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(strip $(TMPDIR))/$(strip $(BIN_FILE)).tmp $(AND) \
		$(MCU_MK_IMAGE_TOOL) $(strip $(TMPDIR))/$(strip $(BIN_FILE)).tmp $(MCU_MK_IMAGE_CFG) > $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(AND) \
		$(IF_EXIST) $(strip $(TMPDIR))/$(strip $(BIN_FILE)).tmp $(THEN) ($(DEL_FILE) $(strip $(TMPDIR))/$(strip $(BIN_FILE)).tmp) $(CMD_END) $(AND) \
...
ifneq ($(strip $(NAND_FLASH_BOOTING)),NONE)
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM  \
		(perl .\tools\CheckBinaryBlockUsage.pl $(TMPDIR)\~flash_cfg_tmp.c $(strip $(TARGDIR))\$(strip $(BIN_FILE)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(TARGDIR))\$(strip $(BTLD_BIN_FILE))  $(strip $(TARGDIR))\$(strip $(BTLD_EXT_BIN_FILE)) $(strip $(PLATFORM)) $(strip $(BUILD_SYSDIR)) "$(strip $(PROJECT_MAKEFILE))" > $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log 2>&1) & \
		(if ERRORLEVEL 1 (echo Error: CheckBinaryBlockUsage Failed!! Please check $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log for more details.))
endif

# Check if memory expiration.
	@if exist $(FIXPATH)\tools\ckmemlayout.pl \
		(if exist $(strip $(TMPDIR))\~lis_temp \
			(perl $(FIXPATH)\tools\ckmemlayout.pl 2 TRUE $(SCATTERFILE) $(strip $(TMPDIR))\~lis_temp $(strip $(TMPDIR))\~flash_cfg_tmp.c "$(strip $(PROJECT_MAKEFILE))" $(strip $(BUILD_SYSDIR)) $(strip $(INSIDE_MTK)) > $(strip $(COMPLOGDIR))\ckmemlayout2.log) & \
...
ifneq ($(strip $(NAND_FLASH_BOOTING)),NONE)
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM  \
		(perl .\tools\CheckBinaryBlockUsage.pl $(strip $(TMPDIR))\~flash_cfg_tmp.c $(strip $(TARGDIR))\$(strip $(BIN_FILE)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(TARGDIR))\$(strip $(BTLD_BIN_FILE)) $(strip $(TARGDIR))\$(strip $(BTLD_EXT_BIN_FILE)) $(strip $(PLATFORM)) $(strip $(BUILD_SYSDIR)) "$(strip $(PROJECT_MAKEFILE))"> $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log 2>&1) & \
...
bootloader: gen_bl_verno gencompbld bl_preflow $(BTLD_BIN_FILE) $(BTLD_EXT_BIN_FILE) ckBLMemLayout done
  else
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(IF_EXIST) tools/CMMAutoGen.pl  $(THEN)\
		(((perl tools/CMMAutoGen.pl 2 $(strip $(PROJDIR))/cmm/BOOTLOADER_$(strip $(PROJECT_NAME))_$(strip $(PLATFORM))_nocode.cmm ../bin/$(BTLD_PREFIX).elf "$(strip $(PROJECT_MAKEFILE))" $(strip $(BIN_FILE)) $(strip $(TMPDIR))/~lis_temp "$(CC)" "$(VIA)" $(strip $(TMPDIR))/~customIncDef.tmp $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))/cmmgen_blnocode.log) $(WITH) \
		($(IF_ERR_TRUE) $(THEN) \
			echo Error: generate BOOTLOADER CMM file Failed. Please check $(strip $(COMPLOGDIR))/cmmgen_blnocode.log $(WITH) exit 1 \
		$(CMD_END))) $(AND) \
		((perl tools/CMMAutoGen.pl 3 $(strip $(PROJDIR))/cmm/EXT_BOOTLOADER_$(strip $(PROJECT_NAME))_$(strip $(PLATFORM))_nocode.cmm ../bin/$(BTLD_EXT_PREFIX).elf "$(strip $(PROJECT_MAKEFILE))" $(strip $(BIN_FILE)) $(strip $(TMPDIR))/~lis_temp "$(CC)" "$(VIA)" $(strip $(TMPDIR))/~customIncDef.tmp $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))/cmmgen_extblnocode.log) $(WITH) \
		($(IF_ERR_TRUE) $(THEN) \
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(IF_EXIST) $(FIXPATH)/tools/ckMemLayout.pl $(THEN)\
		(perl $(FIXPATH)/tools/ckMemLayout.pl 1 TRUE $(SCATTERFILE) $(strip $(TMPDIR))/~lis_temp $(TMPDIR)/~flash_cfg_tmp.c "$(strip $(PROJECT_MAKEFILE))" $(strip $(BUILD_SYSDIR)) $(strip $(INSIDE_MTK)) > $(strip $(COMPLOGDIR))/ckmemlayout1.log) $(CMD_END)
...
ifeq ($(strip $(MBIS_EN)),TRUE)
  ...
  ifeq ($(strip $(COMPILER)),GCC)
		@$(IF_EXIST) tools/CMMAutoGen.pl $(THEN) \
			((perl tools/CMMAutoGen.pl 0 $(strip $(PROJDIR))/cmm/$(strip $(TARGNAME))_nocode.cmm ../bin/$(TARGNAME).elf "$(strip $(PROJECT_MAKEFILE))" $(strip $(BIN_FILE)) $(strip $(TARGDIR))/$(TARGNAME).sym "$(CC)" "$(VIA)" $(strip $(TMPDIR))/~customIncDef.tmp $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))/cmmgen_nocode.log) $(WITH)  \
    ...
    ifeq ($(strip $(RUN_CMMGEN_HQ)),TRUE)
			@$(IF_EXIST) tools/CMMAutoGen.pl  $(THEN)\
				((perl tools/CMMAutoGen.pl 1 $(strip $(PROJDIR))/cmm/$(strip $(TARGNAME))_HQ_Only.cmm ../bin/$(TARGNAME).elf "$(strip $(PROJECT_MAKEFILE))" $(strip $(BIN_FILE)) $(strip $(TARGDIR))/$(TARGNAME).sym "$(CC)" "$(VIA)" $(strip $(TMPDIR))/~customIncDef.tmp $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))/cmmgen_hqonly.log) $(WITH) \
				$(IF_ERR_TRUE) $(THEN) (echo Error: generate CMM file Failed. Please check $(strip $(COMPLOGDIR))/cmmgen_hqonly.log $(WITH) exit 1) $(CMD_END))\
			$(CMD_END)
    endif
  else
		@$(IF_EXIST) tools/CMMAutoGen.pl  $(THEN)\
			((perl tools\CMMAutoGen.pl 0 $(strip $(PROJDIR))\cmm\$(strip $(TARGNAME))_nocode.cmm ..\bin\$(TARGNAME).elf "$(strip $(PROJECT_MAKEFILE))" $(strip $(BIN_FILE)) $(strip $(TARGDIR))\$(TARGNAME).lis "$(CC)" "$(VIA)" $(strip $(TMPDIR))\~customIncDef.tmp $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))\cmmgen_nocode.log) $(WITH)  \
			$(IF_ERR_TRUE) $(THEN) (echo Error: generate CMM file Failed. Please check $(strip $(COMPLOGDIR))/cmmgen_nocode.log $(WITH) exit 1) $(CMD_END))\
		$(CMD_END)
    ifeq ($(strip $(RUN_CMMGEN_HQ)),TRUE)
			@$(IF_EXIST) tools/CMMAutoGen.pl $(THEN) \
				((perl tools\CMMAutoGen.pl 1 $(strip $(PROJDIR))\cmm\$(strip $(TARGNAME))_HQ_Only.cmm ..\bin\$(TARGNAME).elf "$(strip $(PROJECT_MAKEFILE))" $(strip $(BIN_FILE)) $(strip $(TARGDIR))\$(TARGNAME).lis "$(CC)" "$(VIA)" $(strip $(TMPDIR))\~customIncDef.tmp $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))\cmmgen_hqonly.log) $(WITH) \
				$(IF_ERR_TRUE) $(THEN) (echo Error: generate CMM file Failed. Please check $(strip $(COMPLOGDIR))/cmmgen_hqonly.log $(WITH) exit 1) $(CMD_END))\
			$(CMD_END)
    endif
  endif
...
ifneq ($(filter $(strip $(PLATFORM)),$(SV5_PLATFORM)),)
  ...
  ifeq ($(strip $(PARTIAL_SOURCE)),TRUE)
		@$(IF_EXIST) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(THEN)\
			(perl $(FIXPATH)/tools/cfgGen.pl 1 $(strip $(TARGDIR)) $(strip $(BIN_FILE)) $(strip $(BTLD_BIN_FILE)) "$(strip $(PROJECT_MAKEFILE))" $(strip $(BUILD_SYSDIR)) $(SCATTERFILE) "$(CC)" "$(VIA)" $(strip $(TMPDIR))/~customIncDef.tmp "$(strip $(NET_PATH))" $(strip $(TMPDIR)) -sign> $(strip $(COMPLOGDIR))/cfggen.log) \
		 $(CMD_END)
  else
		@$(IF_EXIST) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(THEN)\
			(perl $(FIXPATH)/tools/cfgGen.pl 1 $(strip $(TARGDIR)) $(strip $(BIN_FILE)) $(strip $(BTLD_BIN_FILE)) "$(strip $(PROJECT_MAKEFILE))" $(strip $(BUILD_SYSDIR)) $(SCATTERFILE) "$(CC)" "$(VIA)" $(strip $(TMPDIR))/~customIncDef.tmp "$(strip $(NET_PATH))" $(strip $(TMPDIR))>  $(strip $(COMPLOGDIR))/cfggen.log) \
		$(CMD_END)
  endif
endif
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
	...
	@$(IF_EXIST) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(THEN)\
		perl tools/parse_comp_err.pl $(strip $(PROJECT_NAME)/$(FLAVOR)) $(MODIS_CONFIG) $(strip $(TMPDIR)) $(CMD_END)
